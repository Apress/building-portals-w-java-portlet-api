...
<%!	////////////////
	 ...
	 */
	private String printChildMessage( Forum forum, ForumThread thread, ForumMessage message, int indentation, String mode, HttpServletRequest request, HttpServletResponse response )
	{
		...
		StringBuffer urlpath = new StringBuffer("reply=true&forum=");
		urlpath.append(forumID).append("&thread=").append(threadID).append("&message=").append(messageID);
		
		javax.portlet.PortletURL url = ((javax.portlet.RenderResponse)response).createRenderURL();
        url.setParameter("href","post.jsp");
        url.setParameter("query",urlpath.toString());

		buf.append(url.toString());
		
		buf.append("\"><img src=\"");
		
		// Here we're insering into a scripting variable directly, so we cannot use our custom
		// tag to form the link. We'll make it manually.
		
		String imgPath = 
		   ((javax.portlet.RenderResponse)response).encodeURL(
		      ((javax.portlet.RenderRequest)request).getContextPath() + "/" + "images/reply.gif" );		
		buf.append(imgPath);

		buf.append("\" width=\"50\" height=\"19\" alt=\"Click to reply to this message\" border=\"0\"></a>");
		buf.append("</td>");
	 ...
	 */	
	private String printChildren( TreeWalker walker, Forum forum, ForumThread thread, ForumMessage message, int indentation, String mode, HttpServletRequest request, HttpServletResponse response )
	{
		StringBuffer buf = new StringBuffer();
		
		buf.append( printChildMessage( forum, thread, message, indentation, mode, request, response ) );
        ...
        if( numChildren > 0 ) {
            for( int i=0; i<numChildren; i++ ) {
                buf.append(
					printChildren( walker, forum, thread, walker.getChild(message,i), (indentation+1), mode, request, response )
				);
            }
        }
		return buf.toString();
...
<td><table bgcolor="#cccccc" cellpadding="4" cellspacing="1" border="0" width="100%">
	<tr bgcolor="#eeeeee">
		...
		<td width="98%" align="center">
		    <pb:url  state="NORMAL" mode="VIEW" var="respond">post.jsp?reply=true&forum=<%= forumID %>&thread=<%= threadID %>&message=<%= rootMessageID %></pb:url>
			<a href="<%=respond%>"
			><img src="<pb:href path="images/reply.gif"/>" width="50" height="19" alt="Click to reply to this message" border="0"></a>
		</td>
...
<%	/////////////////////////
	...
	if( numReplies > 0 ) {
		...
		for( int i=0; i<numChildren; i++ ) {
			buf.append( 
				printChildren( treeWalker, forum, thread, treeWalker.getChild(rootMessage,i), indentation, mode, request, response )
			);
		}
%>
		<%= buf.toString() %>
